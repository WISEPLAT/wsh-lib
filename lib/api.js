...
var Api = function Api(provider) {
  ...
  var encodeABI = function encodeABI(type, value) {
    if (type === "bytes") {
      ...
      throw "Wsh-lib can't encode ABI type " + type + " yet.";
    }
  };

  var sendTransaction = send("wsh_sendTransaction");
  var sendRawTransaction = send("wsh_sendRawTransaction");
  var getTransactionReceipt = send("wsh_getTransactionReceipt");
  var compileSolidity = send("wsh_compileSolidity");
  var call = send("wsh_call");
  var getBalance = send("wsh_getBalance");
  var accounts = send("wsh_accounts");
  ...
  var addTransactionDefaults = async function addTransactionDefaults(tx) {
    var baseDefaults = [tx.chainId || send("net_version")(), tx.gasPrice || send("wsh_gasPrice")(), tx.nonce || send("wsh_getTransactionCount")(tx.from, "latest"), tx.value || "0x0", tx.data || "0x"];
    ...
    var gas = tx.gas ? tx.gas : Nat.div(Nat.mul((await send("wsh_estimateGas")({
      from: tx.from,
      to: tx.to,
      value: tx.value,
      nonce: tx.nonce,
