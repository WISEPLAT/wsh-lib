...
const wshjs = {
  signer: require("wshjs-signer"),
  account: require("wshjs-account")};
...
describe("account", function () {
  ...
  it("must match the generated address with WshJS-account", () => {
    F.forall([F.NBytes(32)], pvt => {
      const wshfpAcc = Account.fromPrivate(pvt);
      const wshjsAcc = wshjs.account.privateToAccount(pvt);
      return wshfpAcc.address === wshjsAcc.address;
    }, 128);
  });
    ...
    accounts.forEach(function (correctAccount, i) {
      ...
      correctAccount.transactions.forEach(function (transaction) {
        ...
        if (transaction.oldSignature) {
          ...
          //Signs it, using pre-EIP 155 scheme (using the wshjs-signer lib)
          const oldSignature = wshjs.signer.sign(txObj, correctAccount.privateKey);
